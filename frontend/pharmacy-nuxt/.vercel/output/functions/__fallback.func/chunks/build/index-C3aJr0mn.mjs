    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(unref(products), (p) => {
          var _a2;
          _push(`<tr class="hover:bg-gray-50 dark:hover:bg-slate-700/20 transition-colors"><td class="px-4 py-3.5"><input type="checkbox"${ssrRenderAttr("value", p.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(selected)) ? ssrLooseContain(unref(selected), p.id) : unref(selected)) ? " checked" : ""} class="accent-emerald-600 w-4 h-4"></td><td class="px-4 py-3.5"><div class="flex items-center gap-3"><img${ssrRenderAttr("src", p.primaryImage || "/img/placeholder.png")}${ssrRenderAttr("alt", p.name)} class="w-10 h-10 rounded-lg object-contain bg-gray-50 flex-shrink-0"><div class="min-w-0"><p class="text-sm font-semibold text-gray-900 dark:text-white truncate max-w-[200px]">${ssrInterpolate(p.name)}</p><div class="flex items-center gap-1.5 mt-0.5">`);
...
//# sourceMappingURL=index-C3aJr0mn.mjs.map
