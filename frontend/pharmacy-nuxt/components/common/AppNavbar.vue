<template>
  <header class="sticky top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-b border-gray-100 dark:border-slate-800">
    ...
    <div class="bg-emerald-700 text-white text-xs py-1.5 text-center hidden md:block">
      Free delivery on orders over 300 EGP &nbsp;·&nbsp; Call us: +20 100 000 0000
    </div>
    ...
    <div class="page-container">
      <div class="flex items-center justify-between h-16 gap-4">
        ...
        <div class="flex-1 max-w-xl hidden md:block">
          <div class="relative">
            ...
            <input v-model="searchQuery" @keyup.enter="doSearch"
              type="text" placeholder="Search medicines, vitamins, skincare..."
        ...
        <div class="flex items-center gap-1">
            ...
            <div class="relative" ref="profileRef">
              ...
              <Transition enter-active-class="transition ease-out duration-100"
                ...
                <div v-if="profileOpen"
                  class="absolute right-0 mt-2 w-52 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 overflow-hidden z-50">
                  ...
                  <nav class="p-1.5">
                    <NuxtLink to="/account/profile" @click="profileOpen = false"
                      ...
                      <Icon name="heroicons:user" class="w-4 h-4 text-gray-400" /> My Profile
                    </NuxtLink>
                    <NuxtLink to="/account/orders" @click="profileOpen = false"
                      class="flex items-center gap-2.5 px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-slate-700 rounded-xl transition-colors">
                      <Icon name="heroicons:shopping-bag" class="w-4 h-4 text-gray-400" /> My Orders
                    </NuxtLink>
                    <NuxtLink v-if="authStore.isAdmin || authStore.isPharmacist" to="/admin" @click="profileOpen = false"
                      ...
                      <Icon name="heroicons:chart-bar" class="w-4 h-4" /> Dashboard
                    </NuxtLink>
                    <button @click="logout"
                      class="flex items-center gap-2.5 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-xl transition-colors w-full">
                      <Icon name="heroicons:arrow-right-on-rectangle" class="w-4 h-4" /> Sign Out
                    </button>
                  </nav>
                </div>
              </Transition>
          ...
          <template v-else>
            <NuxtLink to="/auth/login" class="btn-ghost hidden sm:flex text-sm">Sign In</NuxtLink>
        ...
        <div v-if="mobileSearch" class="pb-3 md:hidden">
          <div class="relative">
            ...
            <input v-model="searchQuery" @keyup.enter="doSearch" type="text"
              placeholder="Search medicines..." class="input-field pl-10 py-2.5 text-sm" autofocus />
          </div>
        </div>
      </Transition>
      ...
      <nav class="hidden md:flex items-center gap-1 pb-2 overflow-x-auto">
        <NuxtLink to="/products" class="nav-category-link">All Products</NuxtLink>
